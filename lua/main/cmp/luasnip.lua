...
local cfg = pRequire("main").config.cmp

local pathUtils = require("main.utils.path")
...
pRequire("luasnip.loaders.from_lua").load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "main", "cmp", "snippets", "lua"),
})
pRequire("luasnip.loaders.from_vscode").lazy_load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "main", "cmp", "snippets", "vscode"),
})
